...
public class NMEATalkerIds extends NMEASentences
{
    ...
    {
        for (TalkerId t : TalkerId.values())
        {
            System.err.println("@Rule(left=\"talkerId\", value=\"'" + t.name() + "'\")");
            System.err.println("protected void " + t.name().toLowerCase() + "(@ParserContext(\"data\") NMEAObserver data)");
            System.err.println("{");
            System.err.println("    data.setTalkerId(TalkerId." + t.name() + ");");
            System.err.println("}");
        }
    }
}
