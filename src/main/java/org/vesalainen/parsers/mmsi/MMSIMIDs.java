...
public class MMSIMIDs
{
    ...
    public static void main(String... args)
    {
        ...
        {
            try (
                    FileOutputStream fos = new FileOutputStream("src/main/resources/org/vesalainen/parsers/mmsi/MID.txt");
                    OutputStreamWriter osw = new OutputStreamWriter(fos, "utf-8");
                    PrintWriter pw = new PrintWriter(osw)
                            )
            {
                Collection<ISO3166Entry> allEntries = ISO3166.getAllEntries();
                for (Entry<Integer,String> entry : map.entrySet())
                {
                    pw.println(entry.getKey());
                    pw.println(entry.getValue());
                    boolean found = false;
                    for (ISO3166Entry l : allEntries)
                    {
                        String c = l.getEnglishShortName();
                        if (!c.isEmpty() && entry.getValue().startsWith(c))
                        {
                            pw.println(l.getAlpha2Code());
                            found = true;
                            break;
                        }
                    }
